...
void getSamples(const Function<Y, X>& target, const SampleDistribution<X>&
  ...
  targetSamples.clear();
  targetSamples.reserve(numSamples);
  weights.clear();
  weights.reserve(numSamples);
  double normalizer = 0;
  for (size_t i = 0; i < numSamples; ++i) {
    const X sample = proposal.getSample();
    propSamples.push_back(sample);
    const double weight = target(sample) / proposal(sample);
    weights.push_back(weight);
    normalizer += weight;
  }
  Eigen::Matrix<double, Eigen::Dynamic, 1> probabilities(numSamples);
  static const Randomizer<double, Eigen::Dynamic> randomizer;
  for (size_t i = 0; i < numSamples; ++i) {
